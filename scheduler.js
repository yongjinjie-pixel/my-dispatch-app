...
export function ensureState(raw) {
  ...
  state.schemaVersion = 13; return state;
}
...
export function makeWhatsAppMessage(state, date) {
  ...
  const customerRemark = (item) => [...new Set([state.customers.find((customer) => customer.id === item.customerId)?.notes, item.notes].map(whatsappSafe).filter(Boolean))].join(" · ");
