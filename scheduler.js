...
export function defaultState() {
  ...
  return { schemaVersion: 11, companyName: "My Transport Dispatch", notices: ["Operasi JPJ: check documents, tyre condition and load cover before moving."], trucks, sources, materials, customers, orders, assignments: [], tripLogs: [], extraTripLogs: [], quarryTripLogs: [], quarryEnquiries: [], sandpitEntries: [], sandpitPermits: [], dailyFleetPlans: [], messageRemarks: [], makeUps: [], settings: { storageMode: "phone-only", lastBackupAt: "", sandpitPermitCost: 0, themePreference: "system", screenOrientation: "auto" }, updatedAt: now };
}

export function ensureState(raw) {
  const seed = defaultState(); if (!raw || !Array.isArray(raw.trucks)) return seed;
  const state = {
    ...seed, ...raw, settings: { ...seed.settings, ...(raw.settings || {}), lastBackupAt: raw.settings?.lastBackupAt || "", sandpitPermitCost: number(raw.settings?.sandpitPermitCost), screenOrientation: Number(raw.schemaVersion || 0) < 11 ? "auto" : (raw.settings?.screenOrientation || "auto") }, notices: Array.isArray(raw.notices) ? raw.notices : seed.notices,
  ...
  state.schemaVersion = 11; return state;
}
...
export function addManualOrderLoad(state, { date, orderId, truckId }) {
  ...
  return { ...state, assignments: renumberAssignments([...state.assignments, assignment]), updatedAt: new Date().toISOString() };
}
...
function firstQuarryMaterial(state, sourceId = "") {
  ...
  return { sourceId: source.id, materialId: state.materials.find((item) => item.sourceId === source.id)?.id || "" };
}
...
export function makeWhatsAppMessage(state, date) {
  ...
  const manualSandpitEntries = state.sandpitEntries || [];
  const sandpitSourceIds = new Set([...normalSand, ...jagung].map((item) => item.sourceId).filter((id) => sourceById(state, id)?.type === "Sandpit")); manualSandpitEntries.filter((entry) => entry.date === date && sourceById(state, entry.sourceId)?.type === "Sandpit").forEach((entry) => sandpitSourceIds.add(entry.sourceId));
  const sandSources = sandpitSourceIds.size ? state.sources.filter((source) => source.type === "Sandpit").map((source) => { const planned = assignments.filter((item) => normalise(item.kind) === "sand" && item.sourceId === source.id); const entered = manualSandpitEntries.filter((entry) => entry.date === date && entry.sourceId === source.id).map((entry) => ({ truckId: entry.truckId })); const unique = new Map(); [...planned, ...entered].forEach((item) => unique.set(item.truckId, item)); return `Masuk Lombong ${source.name}: ${truckCounts([...unique.values()], truckNo)}`; }) : [];
  const groupedLines = (items, type) => { const groups = new Map(); items.forEach((item) => { const material = materialById(state, item.materialId); const key = `${item.customerId}|${material?.name || item.product || item.productLabel}|${item.notes || ""}`; groups.set(key, [...(groups.get(key) || []), item]); }); return [...groups.values()].map((group) => { const sample = group[0]; const material = materialById(state, sample.materialId); if (type === "sand") { const name = material?.name || sample.product || sample.productLabel; const sourceText = customerMaterialSources(state, sample.customerId, name) || sourceShort(sourceById(state, material?.sourceId || sample.sourceId)?.name); return `${customerName(sample)}:\n${name} ${sourceText} - ${group.length} (${truckCounts(group, truckNo)})${whatsappNote(sample.notes) ? `\n${whatsappNote(sample.notes)}` : ""}`; } const source = sourceById(state, material?.sourceId || sample.sourceId)?.name || sample.source || "Quarry"; return `${source} ke ${customerName(sample)}:\n${material?.name || sample.product || sample.productLabel} - ${group.length} (${truckCounts(group, truckNo)})${whatsappNote(sample.notes) ? `\n${whatsappNote(sample.notes)}` : ""}`; }); };
