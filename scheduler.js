...
const materialSeed = [
  ...
  ["mat-gsl-23x", "src-gsl", "2x/3x cuci", "Sand"], ["mat-ml-kasar", "src-ml", "Pasir Kasar", "Sand"],
  ["mat-ml-serdang", "src-ml", "Pasir Serdang", "Sand"], ["mat-gsl-halus", "src-gsl", "Pasir Halus", "Sand"],
...
export function defaultState() {
  ...
  const customers = customerRows.map(([name, kind, defaultZone, rule]) => ({
    id: `customer-${name.replace(/[^a-z0-9]/gi, "-").toLowerCase()}`, name, kind, defaultZone,
    materialIds: [...(materialRefs[rule] || [])], notes: "", updatedAt: new Date().toISOString(),
  }));
  const orderRows = [
  ...
  const orders = orderRows.map(([name, loads, notes = "", early = false]) => {
    ...
    return makeOrderFromMaterial({ id: uid("order"), date, customerId: target.id, loads, zone: target.defaultZone, kind: target.kind, materialId: target.materialIds[0], early, notes }, { sources, materials, customers });
  });
  return {
    schemaVersion: 3, companyName: "My Transport Dispatch", notices: ["Operasi JPJ: check documents, tyre condition and load cover before moving."],
    trucks, sources, materials, customers, orders, assignments: [], tripLogs: [], makeUps: [],
    settings: { endpoint: "", syncSecret: "", lastSyncAt: "" }, updatedAt: new Date().toISOString(),
  };
}
...
export function makeOrderFromMaterial(order, state) {
  ...
  const materialId = order.materialId || customer?.materialIds?.[0] || "";
...
export function ensureState(raw) {
  ...
  const state = {
    ...seed, ...raw, settings: { ...seed.settings, ...(raw.settings || {}) }, notices: Array.isArray(raw.notices) ? raw.notices : seed.notices,
  ...
  state.customers = state.customers.map((customer) => {
    ...
    return { ...customer, kind: customer.kind || "Sand", defaultZone: customer.defaultZone || "Near", materialIds: [...new Set(materialIds)], notes: customer.notes || "" };
  });
