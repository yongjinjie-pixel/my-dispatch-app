...
export function defaultState() {
  ...
  return { schemaVersion: 14, companyName: "My Transport Dispatch", notices: ["Operasi JPJ: check documents, tyre condition and load cover before moving."], trucks, sources, materials, customers, orders, assignments: [], tripLogs: [], extraTripLogs: [], quarryTripLogs: [], quarryEnquiries: [], quarryRequests: [], quarryOnlyEntries: [], sandpitEntries: [], sandpitPermits: [], dailyFleetPlans: [], messageRemarks: [], driverAbsences: [], makeUps: [], settings: { storageMode: "phone-only", lastBackupAt: "", sandpitPermitCost: 0, themePreference: "system", screenOrientation: "auto" }, updatedAt: now };
}
...
export function ensureState(raw) {
  ...
  state.orders = state.orders.map((order) => {
    ...
    return makeOrderFromMaterial({ ...order, sequence: number(order.sequence) || nextSequence, materialId, materialOverride }, state);
  });
  ...
  state.schemaVersion = 14; return state;
}
...
export function markDriverAbsent(state, { date, truckId, remark = "" }) {
  const now = new Date().toISOString(); const skipped = state.assignments.filter((item) => item.date === date && item.truckId === truckId && normalise(item.status) === "planned");
...
export function makeWhatsAppMessage(state, date) {
  ...
  const groupedLines = (items, type) => { const groups = new Map(); items.forEach((item) => { const material = materialById(state, item.materialId); const key = `${item.customerId}|${material?.name || item.product || item.productLabel}|${customerRemark(item)}`; groups.set(key, [...(groups.get(key) || []), item]); }); return [...groups.values()].sort((left, right) => number(state.orders.find((order) => order.id === left[0].orderId)?.sequence) - number(state.orders.find((order) => order.id === right[0].orderId)?.sequence)).map((group) => { const sample = group[0]; const material = materialById(state, sample.materialId); const note = customerRemark(sample); if (type === "sand") { const name = material?.name || sample.product || sample.productLabel; const sourceText = customerMaterialSources(state, sample.customerId, name) || sourceShort(sourceById(state, material?.sourceId || sample.sourceId)?.name); return `${customerName(sample)}:\n${name} ${sourceText} - ${group.length} (${truckCounts(group, truckNo)})${whatsappNote(note) ? `\n${whatsappNote(note)}` : ""}`; } const source = sourceById(state, material?.sourceId || sample.sourceId)?.name || sample.source || "Quarry"; return `${source} ke ${customerName(sample)}:\n${material?.name || sample.product || sample.productLabel} - ${group.length} (${truckCounts(group, truckNo)})${whatsappNote(note) ? `\n${whatsappNote(note)}` : ""}`; }); };
