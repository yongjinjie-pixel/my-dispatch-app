...
export function defaultState() {
  ...
  return { schemaVersion: 12, companyName: "My Transport Dispatch", notices: ["Operasi JPJ: check documents, tyre condition and load cover before moving."], trucks, sources, materials, customers, orders, assignments: [], tripLogs: [], extraTripLogs: [], quarryTripLogs: [], quarryEnquiries: [], quarryRequests: [], quarryOnlyEntries: [], sandpitEntries: [], sandpitPermits: [], dailyFleetPlans: [], messageRemarks: [], makeUps: [], settings: { storageMode: "phone-only", lastBackupAt: "", sandpitPermitCost: 0, themePreference: "system", screenOrientation: "auto" }, updatedAt: now };
}
...
export function ensureState(raw) {
  ...
  state.orders = state.orders.map((order) => {
    ...
    return makeOrderFromMaterial({ ...order, sequence: number(order.sequence) || nextSequence, materialId, materialOverride: typeof order.materialOverride === "boolean" ? order.materialOverride : Boolean(materialId && materialId !== defaultCustomerMaterialId(state, target)) }, state);
  });
  ...
  state.schemaVersion = 12; return state;
}
...
export function tallyForDate(state, date) {
  const lines = new Map(); state.orders.filter((item) => item.date === date).forEach((item) => { const line = lines.get(item.customerId) || { customerId: item.customerId, required: 0, planned: 0 }; line.required += number(item.loads); lines.set(item.customerId, line); }); state.assignments.filter((item) => item.date === date && item.status !== "Skipped" && item.customerId).forEach((item) => { const line = lines.get(item.customerId) || { customerId: item.customerId, required: 0, planned: 0 }; line.planned += 1; lines.set(item.customerId, line); });
...
export function makeWhatsAppMessage(state, date) {
  ...
  const quarryOnly = state.trucks.filter((truck) => quarryOnlyIds.has(truck.id)).map(truckLabel).join("/");
