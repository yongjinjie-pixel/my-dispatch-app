...
export function defaultState() {
  ...
  const sources = sourceSeed.map(([id, name, type]) => ({ id, name, type, notes: "", weighbridgeContact: "", quarryNote: "", updatedAt: now }));
  ...
  return { schemaVersion: 8, companyName: "My Transport Dispatch", notices: ["Operasi JPJ: check documents, tyre condition and load cover before moving."], trucks, sources, materials, customers, orders, assignments: [], tripLogs: [], quarryTripLogs: [], quarryEnquiries: [], makeUps: [], settings: { storageMode: "phone-only", lastBackupAt: "", sandpitPermitCost: 0 }, updatedAt: now };
}
...
export function ensureState(raw) {
  ...
  state.schemaVersion = 8; return state;
}
...
export function sandpitPermitPlan(state, date) {
  ...
  // A permit becomes worthwhile at three sand loads, but these are the
  // high-capacity targets: 5 Near or 3 Near + 1 Far.  Keeping the threshold
  // separate from the capacity prevents the app from wrongly calling 2N+1F a
  // truck's maximum.
  const farGroups = farLoads;
  const nearGroups = nearLoads > farGroups * 3 ? Math.ceil((nearLoads - farGroups * 3) / 5) : 0;
  const groups = [
    ...Array.from({ length: farGroups }, () => ({ near: 3, far: 1, stone: 0, minimumNear: 2, minimumFar: 1, label: "3 Near + 1 Far" })),
    ...Array.from({ length: nearGroups }, () => ({ near: 5, far: 0, stone: 0, minimumNear: 3, minimumFar: 0, label: "5 Near" })),
  ];
  const targetNearLoads = farGroups * 3 + nearGroups * 5;
  ...
  return { nearLoads, farLoads, targetNearLoads, minimumNearLoads, minimumExtraNearLoads: Math.max(0, minimumNearLoads - nearLoads), fullCapacityExtraNearLoads: Math.max(0, targetNearLoads - nearLoads), extraNearLoads: Math.max(0, targetNearLoads - nearLoads), groups, groupCount: groups.length, eligibleTruckCount: eligibleTrucks.length, workingTruckCount: workingTrucks.length, markedQuarryOnly, suggestedQuarryOnlyCount, recommendedQuarryOnlyTruckIds };
}
...
export function financialSummary(state, date) {
  ...
  const assignedPermitTrucks = new Set(state.assignments.filter((item) => item.date === date && sourceTypeFor(state, item) === "Sandpit").map((item) => item.truckId));
  const permitCount = assignedPermitTrucks.size || permitPlan.groupCount;
  const permitCost = number(state.settings?.sandpitPermitCost) * permitCount;
  ...
  return { customerMargin, permitCount, permitCost, netAfterPermit: customerMargin - permitCost, configuredOrderLoads, totalOrderLoads: orders.reduce((sum, item) => sum + number(item.loads), 0) };
}
