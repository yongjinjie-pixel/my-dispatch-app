...
export function defaultState() {
  ...
  return { schemaVersion: 10, companyName: "My Transport Dispatch", notices: ["Operasi JPJ: check documents, tyre condition and load cover before moving."], trucks, sources, materials, customers, orders, assignments: [], tripLogs: [], extraTripLogs: [], quarryTripLogs: [], quarryEnquiries: [], sandpitEntries: [], sandpitPermits: [], dailyFleetPlans: [], makeUps: [], settings: { storageMode: "phone-only", lastBackupAt: "", sandpitPermitCost: 0, themePreference: "system", screenOrientation: "auto" }, updatedAt: now };
}
...
export function ensureState(raw) {
  ...
  const state = {
    ...
    customers: Array.isArray(raw.customers) ? raw.customers : seed.customers, orders: Array.isArray(raw.orders) ? raw.orders : [], assignments: Array.isArray(raw.assignments) ? raw.assignments : [], tripLogs: Array.isArray(raw.tripLogs) ? raw.tripLogs : [], extraTripLogs: Array.isArray(raw.extraTripLogs) ? raw.extraTripLogs : [], quarryTripLogs: Array.isArray(raw.quarryTripLogs) ? raw.quarryTripLogs : [], quarryEnquiries: Array.isArray(raw.quarryEnquiries) ? raw.quarryEnquiries : [], sandpitEntries: Array.isArray(raw.sandpitEntries) ? raw.sandpitEntries : [], sandpitPermits: Array.isArray(raw.sandpitPermits) ? raw.sandpitPermits : [], dailyFleetPlans: Array.isArray(raw.dailyFleetPlans) ? raw.dailyFleetPlans : [], makeUps: Array.isArray(raw.makeUps) ? raw.makeUps : [],
  };
  ...
  state.schemaVersion = 10; return state;
}
...
export function makeWhatsAppMessage(state, date) {
  ...
  const message = ["Esok order pasir", sandSources.join("\n"), groupedLines(normalSand, "sand").join("\n\n") || "-", jagung.length ? "---------------------------------------------------" : "", jagung.length ? "Esok order pasir jagung" : "", jagung.length ? groupedLines(jagung, "sand").join("\n\n") : "", "---------------------------------------------------", "Esok order batu", `Esok masuk quarry sahaja: ${quarryOnly || "-"}`, groupedLines(stone, "stone").join("\n\n") || "-", quarryEnquiries.join("\n\n"), notices ? `Peringatan semua driver:\n${notices}` : ""].filter((line, index, list) => line || (index > 0 && list[index - 1])).join("\n\n");
