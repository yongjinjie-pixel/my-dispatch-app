...
// Availability is date-specific: yesterday's Off/Breakdown never blocks a
// truck from tomorrow's planning unless it is marked unavailable again.
export function dailyTruckStatusForDate(state, truck, date) {
  const saved = (state.dailyTruckStatuses || []).find((entry) => entry.date === date && entry.truckId === truck?.id);
  if (saved) return { status: saved.status, statusReason: saved.statusReason || "", repairTime: saved.repairTime === "Afternoon" ? "Afternoon" : "Morning", earlyRepair: Boolean(saved.earlyRepair) };
  return { status: "Available", statusReason: "", repairTime: truck?.repairTime === "Afternoon" ? "Afternoon" : "Morning", earlyRepair: false };
}
...
export function ensureState(raw) {
  ...
  state.schemaVersion = 18; return state;
}
...
// Creates one clear replacement order for a skipped company delivery. The UI
// chooses the next dispatch date and then navigates to it; keeping the data
// change here makes restore, testing and a future SQLite API use the same rule.
export function carrySkippedDelivery(state, { assignmentId, targetDate }) {
  const assignment = state.assignments.find((item) => item.id === assignmentId); const log = state.tripLogs.find((item) => item.assignmentId === assignmentId);
  const skipped = normalise(log?.status || assignment?.status) === "skipped";
  if (!assignment || !targetDate || !skipped || assignment.quarryRequestId) return { state, carried: null, reason: "Only a skipped customer delivery can be carried forward" };
  const existing = state.orders.find((item) => item.carriedFromAssignmentId === assignmentId);
  if (existing) return { state, carried: existing, alreadyCarried: true };
  const customer = state.customers.find((item) => item.id === assignment.customerId); if (!customer) return { state, carried: null, reason: "Customer is no longer available" };
  const original = state.orders.find((item) => item.id === assignment.orderId); const allowed = customerAllowedMaterialIds(state, customer); const override = Boolean(original?.materialOverride); const materialId = override && allowed.includes(original?.materialId) ? original.materialId : (allowed.includes(assignment.materialId) ? assignment.materialId : defaultCustomerMaterialId(state, customer));
  if (!materialId) return { state, carried: null, reason: "This customer needs a valid material rule" };
  const sequence = Math.max(0, ...state.orders.filter((item) => item.date === targetDate).map((item) => number(item.sequence))) + 1;
  const carried = makeOrderFromMaterial({ id: uid("order"), date: targetDate, customerId: assignment.customerId, loads: 1, zone: assignment.zone || customer.defaultZone, kind: assignment.kind || customer.kind, materialId, materialOverride: override, early: Boolean(assignment.early), deliveryWindow: original?.deliveryWindow || assignment.deliveryWindow || "", notes: original?.notes || assignment.notes || "", sequence, carriedFromAssignmentId: assignment.id, carriedFromDate: assignment.date }, state);
  return { state: { ...state, orders: [...state.orders, carried], makeUps: (state.makeUps || []).map((item) => item.assignmentId === assignment.id ? { ...item, status: "Carried", carriedOrderId: carried.id, carriedToDate: targetDate } : item), updatedAt: new Date().toISOString() }, carried };
}
