...
const STORAGE_KEY = "dispatch-pilot-state-v4";
const LEGACY_STORAGE_KEYS = ["dispatch-pilot-state-v3", "dispatch-pilot-state-v1"];
...
function readState() {
  try {
    const raw = JSON.parse(localStorage.getItem(STORAGE_KEY) || LEGACY_STORAGE_KEYS.map((key) => localStorage.getItem(key)).find(Boolean) || "null");
...
function settingsPage() {
  return `<section class="page-title"><p class="eyebrow">MASTER DATA AND LOCAL STORAGE</p><h2>Setup</h2><p>Manage the sources, materials, customers and trucks that the app uses every night.</p></section>
    ...
    <section class="panel local-storage-panel"><p class="eyebrow">GUI PHASE - PHONE ONLY</p><h2>Local data and handover</h2><p>Your records are saved only in this phone's browser. Google Sheets is not used as a database. Download a backup before changing phone, clearing Chrome data or starting the later SQLite API.</p><div class="button-row"><button class="primary" data-action="download-backup">Download JSON backup</button><label class="secondary file-button">Restore JSON backup<input id="backup-file" type="file" accept="application/json,.json"></label></div><small>Future API: normal SQLite CRUD endpoints. The developer handover files in this release define the tables and endpoints.</small></section>`;
}
...
if ("serviceWorker" in navigator) window.addEventListener("load", () => navigator.serviceWorker.register("./sw.js?v=20260713-1", { scope: "./", updateViaCache: "none" }));
render();
